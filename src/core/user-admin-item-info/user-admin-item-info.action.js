...
export function reloadUserAdminItemData() {
  return async (dispatch) => {
    dispatch({
      type: USER_ADMIN_ITEM_INFO_ACTION_TYPE.REQUEST_UPDATE_PENDING,
    });
    ...
    try {
      ...
      dispatch({
        type: USER_ADMIN_ITEM_INFO_ACTION_TYPE.REQUEST_UPDATE_SUCCESS,
        payload: data,
      });
    } catch (error) {
      if (error) {
