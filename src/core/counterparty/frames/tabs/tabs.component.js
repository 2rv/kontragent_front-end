...
export function TabsComponent({ tabsList, activeTabId, activeTabHandler }) {
  return (
    <Container>
      <TabsItemContainer>
        {(tabsList[tabsList.length - 1].hasDangerNotices ? tabsList : tabsList.slice(0, -1)).map((tab) => (
          <TabsItemContent
            key={tab.id}
...
const TabsItemContent = styled.div`
  ...
  `};
  ${(props) => props.hasDangerNotices && `
    span {
      color: ${THEME_COLOR.TEXT.VALIDATION} !important
    }
  `};
`;
