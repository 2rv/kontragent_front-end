...
export function CompanyAccountItemRevisionSelfItemAccordionComponent(props) {
  const { data } = props;

  return data.map((self, key) => (
    <Fragment key={key}>
      <Accordion disableGutters elevation={0} sx={{ p: 0 }}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography variant="title" children={self[DATA_NAME.NAME]} />
        </AccordionSummary>
        <AccordionDetails>
          <Grid container spacing={6}>
            <Grid item>
              ...
              <Typography
                variant="listContent"
                children={self[DATA_NAME.DESCRIPRION]}
              />
            </Grid>
            <Grid item>
              <Typography
              ...
              />
              {self[DATA_NAME.FILES].length ? (
                <FileListElement list={self[DATA_NAME.FILES]} />
              ) : (
                <Typography
                  variant="listContent"
                  children={text(
            ...
            <Grid item container>
              ...
              {self[DATA_NAME.PERIOD].map((item, key) => (
                <Grid item>
