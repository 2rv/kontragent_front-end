...
import { KontragentImportComponent } from './kontragent-import.component';
...
function kontragentsImportReducer(state, action) {
  switch (action.type) {
    case 'PENDING':
      return {
        ...state,
...
export function KontragentImportContainer() {
  const companyId = getQuery('companyId');
  const [state, setState] = useReducer(kontragentsImportReducer, initialState);
  ...
  function onSave() {
    importKontragentsAction(state.data);
  }

  return (
    <KontragentImportComponent
      state={state}
      onSave={onSave}
      onDelete={onDelete}
      onChange={onChange}
